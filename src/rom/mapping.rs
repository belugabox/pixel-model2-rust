...
use super::loader::{LoadedRom, RomSet};
...
impl RomMemoryMapper {
    ...
    fn map_rom_to_memory(
        ...
        let final_address =
            base_address + (loaded_rom.info.bank as u32 * self.mapping_config.bank_size);

        println!(
            "Mapping ROM {} ({}) vers 0x{:08X} ({} octets)",
            rom_name,
        ...
        if loaded_rom.data.len() > self.mapping_config.bank_size as usize {
            return Err(anyhow!(
                "ROM {} trop grande pour une banque ({} > {})",
                rom_name,
                loaded_rom.data.len(),
                self.mapping_config.bank_size
            ));
        }
        ...
        self.mapped_data
            .insert(final_address, loaded_rom.data.clone());
    ...
    fn calculate_base_address(&self, rom_type: &RomType) -> u32 {
        match rom_type {
            ...
            RomType::Graphics | RomType::Geometry | RomType::Texture => {
                self.mapping_config.graphics_rom_base
            }
            RomType::Sound | RomType::Samples => self.mapping_config.audio_rom_base,
            RomType::Data | RomType::Config | RomType::Microcode => {
                self.mapping_config.data_rom_base
            }
        }
    }

    /// Configure le mapping spécifique aux ROMs programme
    fn setup_program_rom_mapping(
        &self,
        base_address: u32,
        data: &[u8],
        ...
        if data.len() >= 1024 {
            ...
            if stack_pointer < 0x10000000
                && reset_vector >= base_address
                && reset_vector < base_address + data.len() as u32
            {
    ...
    fn setup_graphics_rom_mapping(
        ...
        println!(
            "  Estimation: {} chunks de textures, {} chunks de sprites",
            texture_count, sprite_count
        );
        ...
        if let Some(info) = self.get_mapping_info() {
            ...
            for i in 1..sorted_regions.len() {
                ...
                if addr1 + *size1 as u32 > *addr2 {
                    report
                        .errors
                        .push(format!("Chevauchement détecté avec ROM {}", name2));
            ...
            for (_, _, size, rom_type) in &info.regions {
                match rom_type {
                    ...
                    RomType::Graphics | RomType::Geometry | RomType::Texture => {
                        report.statistics.graphics_size += size
                    }
                    RomType::Sound | RomType::Samples => report.statistics.audio_size += size,
                    RomType::Data | RomType::Config | RomType::Microcode => {
                        report.statistics.data_size += size
                    }
                }
            }
        } else {
...
mod tests {
    ...
    fn test_calculate_base_address() {
        ...
        assert_eq!(
            mapper.calculate_base_address(&RomType::Graphics),
            0x08000000
        );
