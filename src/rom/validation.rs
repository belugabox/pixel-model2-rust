    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ValidationError::InvalidCrc32 { expected, found } => {
                write!(f, "CRC32 incorrect: attendu {:#010x}, trouvé {:#010x}", expected, found)
            },
            ...
            ValidationError::InvalidSize { expected, found } => {
                write!(f, "Taille incorrect: attendu {} octets, trouvé {} octets", expected, found)
            },
...
impl RomValidator {
    ...
    pub fn validate_rom_set(rom_files: &[(String, Vec<u8>)], game_info: &GameInfo) -> Result<Vec<(String, ValidationResult)>> {
        let mut results = Vec::new();
        
        // Vérifier chaque ROM requise
        for required_rom in &game_info.required_roms {
            if let Some((_, data)) = rom_files.iter().find(|(name, _)| name == &required_rom.filename) {
        ...
        for optional_rom in &game_info.optional_roms {
            if let Some((_, data)) = rom_files.iter().find(|(name, _)| name == &optional_rom.filename) {
    ...
    pub fn detect_rom_type(data: &[u8], filename: &str) -> super::database::RomType {
        ...
        if filename_lower.contains("gfx") || filename_lower.contains("chr") || filename_lower.contains("spr") {
            return RomType::Graphics;
        }
        if filename_lower.contains("snd") || filename_lower.contains("pcm") || filename_lower.contains("wav") {
    ...
    fn validate_rom_type_content(data: &[u8], rom_type: &super::database::RomType) -> Option<String> {
        use super::database::RomType;
    ...
    pub fn generate_validation_report(results: &[(String, ValidationResult)]) -> String {
        ...
        for (filename, result) in results {
            ...
            report.push_str(&format!("  Statut: {}\n", if result.is_valid { "VALIDE" } else { "INVALIDE" }));
        ...
        report.push_str(&format!("RÉSUMÉ: {}/{} ROMs valides\n", valid_roms, total_roms));
...
mod tests {
    ...
    fn test_rom_type_detection() {
        ...
        assert_eq!(RomValidator::detect_rom_type(&program_data, "program.bin"), RomType::Program);
        
        let graphics_data = vec![0; 0x300000]; // Large ROM -> Graphics
        assert_eq!(RomValidator::detect_rom_type(&graphics_data, "graphics.bin"), RomType::Graphics);
    }
