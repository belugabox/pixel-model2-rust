...
impl RomDecompressor {
    ...
    pub fn sort_rom_files(mut files: Vec<(String, Vec<u8>)>) -> Vec<(String, Vec<u8>)> {
        files.sort_by(|a, b| Self::extract_rom_number(&a.0).cmp(&Self::extract_rom_number(&b.0)));
        files
    }
    ...
    fn extract_rom_number(filename: &str) -> u32 {
        ...
        if let Some(pos) = filename_lower.find("ic") {
            if let Ok(num) = filename_lower[pos + 2..]
                .split('.')
                .next()
                .unwrap_or("0")
                .parse::<u32>()
        ...
        for prefix in &["rom", "prg", "gfx", "snd"] {
            if let Some(pos) = filename_lower.find(prefix) {
                ...
                if let Ok(num) = filename_lower[start..]
                    .chars()
...
mod tests {
    ...
    fn test_compression_detection() {
        assert_eq!(
            RomDecompressor::detect_compression_type(Path::new("test.zip")),
            CompressionType::Zip
        );
        assert_eq!(
            RomDecompressor::detect_compression_type(Path::new("test.bin")),
            CompressionType::None
        );
        assert_eq!(
            RomDecompressor::detect_compression_type(Path::new("test.gz")),
