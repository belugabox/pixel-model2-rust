...
use crate::rom::{Model2MemoryConfig, Model2RomSystem};
...
fn test_complete_rom_system() -> Result<()> {
    ...
    match rom_system.load_and_map_game("virtua_fighter_2", &mut memory) {
        ...
        Err(e) => {
            ...
            assert!(
                e.to_string().contains("ROM non trouvée")
                    || e.to_string().contains("Jeu non trouvé")
            );
        }
    }
...
fn test_mapping_validation() -> Result<()> {
    ...
    assert!(validation
        .warnings
        .contains(&"Aucune ROM mappée".to_string()));
