...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "  U16: Écrit 0x1234, lu 0x{:04X} - {}",
        value,
        if value == 0x1234 { "✓" } else { "✗" }
    );
    ...
    println!(
        "  U32: Écrit 0x12345678, lu 0x{:08X} - {}",
        value,
        if value == 0x12345678 { "✓" } else { "✗" }
    );
    ...
    println!(
        "  GPR[0]: Écrit 0xDEADBEEF, lu 0x{:08X} - {}",
        reg_value,
        if reg_value == 0xDEADBEEF {
            "✓"
        } else {
            "✗"
        }
