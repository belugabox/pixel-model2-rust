...
impl MemoryInterface for Model2Memory {
    fn read_u8(&self, address: u32) -> Result<u8> {
        ...
        let result = if let Some((region, offset)) = self.mapping.resolve_address(address) {
            match region {
    ...
    fn write_u8(&mut self, address: u32, value: u8) -> Result<()> {
        // Déterminer la région mémoire et l'offset
        if let Some((region, offset)) = self.mapping.resolve_address(address) {
            match region {
                ...
                MemoryRegion::MainRom | MemoryRegion::AudioRom => {
