...
use crate::{
    ...
    gpu::Model2Gpu,
    input::InputManager,
    memory::{interface::MemoryInterface, GpuCommand, Model2Memory},
    rom::Model2RomSystem,
};
use anyhow::Result;
use std::sync::Arc;
...
impl AppState {
    ...
    pub fn run_frame(&mut self, mut gpu: Option<&mut Model2Gpu>) -> Result<()> {
        if self.app.running && !self.app.paused {
            ...
            let executed_cycles = self
                .app
                .cpu
                .run_cycles(CYCLES_PER_FRAME, &mut self.app.memory)?;

            // Mettre à jour les registres I/O avec les cycles exécutés
            self.app
                .memory
                .update_io_registers(executed_cycles, &mut self.app.cpu);
            ...
            if !command_batches.is_empty() {
                if let Some(gpu_ref) = gpu.as_mut() {
                    ...
                    println!(
                        "GPU: {} commandes reçues mais GPU non initialisé",
                        command_batches.len()
                    );
                }
            }
            ...
            if executed_cycles > 0 {
                ...
                println!(
                    "GPU Buffer: {} lots traités, taille moyenne {:.1}, max {}",
                    buffer_stats.batches_processed,
                    buffer_stats.average_batch_size,
                    buffer_stats.max_batch_size
                );
            }
        }
        Ok(())
    ...
    fn process_gpu_command(&mut self, command: &GpuCommand, gpu: &mut Model2Gpu) -> Result<()> {
        match command {
            GpuCommand::ClearScreen {
                ...
                println!(
                    "GPU: Clear screen avec couleur [{:.2}, {:.2}, {:.2}, {:.2}]",
            ...
            GpuCommand::SetRenderState { state, enabled } => {
                ...
                let render_state = match state {
                    ...
                    crate::memory::RenderStateType::Transparency => {
                        crate::gpu::RenderState::Transparency
                    }
    ...
    fn convert_gpu_vertices_to_triangle(
        &self,
        vertices: &[crate::memory::GpuVertex; 3],
        texture_id: Option<u32>,
    ) -> crate::gpu::geometry::Triangle3D {
...
impl EmulatorApp {
    ...
    pub fn load_rom(&mut self, game_name: &str) -> Result<()> {
        ...
        self.rom_system
            .load_and_map_game(game_name, &mut self.memory)?;
