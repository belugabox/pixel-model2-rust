...
use pixel_model2_rust::{cpu::NecV60, memory::Model2Memory};
...
fn benchmark_cpu_execution(c: &mut Criterion) {
    ...
    c.bench_function("cpu_single_step", |b| {
        b.iter(|| cpu.step(black_box(&mut memory)).unwrap())
    });

    c.bench_function("cpu_1000_cycles", |b| {
        b.iter(|| {
            cpu.run_cycles(black_box(1000), black_box(&mut memory))
                .unwrap()
        })
    });
}
...
fn benchmark_instruction_decoding(c: &mut Criterion) {
    ...
    c.bench_function("instruction_decode", |b| {
        b.iter(|| decode_instruction(black_box(0x12345678), black_box(0x00000000)))
    });
}
...
criterion_group!(
    ...
    benchmark_instruction_decoding
);
