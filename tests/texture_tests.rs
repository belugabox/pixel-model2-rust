...
use pixel_model2_rust::gpu::texture::{SegaTextureFormat, TextureDecodeParams, TextureManager};
use std::sync::Arc;
...
async fn test_load_simple_rgba8_texture() {
    ...
    assert!(
        result.is_ok(),
        "Erreur lors du chargement texture RGBA8: {:?}",
        result.err()
    );
...
async fn test_multiple_textures_management() {
    ...
    texture_manager
        .load_texture_from_rom(20, &rgb565_data, params)
        .unwrap();
